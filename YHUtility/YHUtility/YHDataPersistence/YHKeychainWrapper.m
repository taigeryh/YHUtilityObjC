...
@implementation YHKeychainWrapper
...
- (void)mySetObject:(id)inObject forKey:(id)key
{
    ...
    {
        [self.keychainData setObject:inObject forKey:key];
...
{
    if (!self.keychainData) //Allocate the keychainData dictionary if it doesn't exist yet.
    {
        self.keychainData = [[NSMutableDictionary alloc] init];
    }
    else if (self.keychainData)
    {
    ...
    [self.keychainData setObject:@"Your comment here." forKey:(__bridge id)kSecAttrComment];
...
- (void)writeToKeychain
{
    ...
    else
    {
        ...
        NSDictionary * addItem = [self dictionaryToSecItemFormat:self.keychainData];
        OSStatus errorcode = SecItemAdd(
                                        (__bridge CFDictionaryRef)addItem,
                                        NULL);
